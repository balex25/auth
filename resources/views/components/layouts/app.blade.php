...
<body id="auth-body" class="overflow-hidden relative w-screen h-screen" style="background-color:{{ config('devdojo.auth.appearance.background.color') }}">
    ...
    <div x-data data-auth="{{ $dyanicPageId }}" class="relative w-full h-full" x-cloak>
        ...
        <a href="{{ \Devdojo\Auth\Helper::localizedUrl('/') }}" class="hidden sm:flex fixed bottom-5 right-5 z-40 justify-center items-center px-2.5 py-1.5 w-auto text-xs font-medium rounded-lg border border-white/20 cursor-pointer text-white backdrop-blur-lg-md bg-white/10 hover:bg-white/30 shadow-md transition-all active:scale-95">
