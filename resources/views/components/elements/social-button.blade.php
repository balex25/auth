<a href="{{ \Devdojo\Auth\Helper::localizedUrl('auth/' . $slug . '/redirect', true) }}" class="flex @if(config('devdojo.auth.settings.center_align_social_provider_button_content')){{ 'justify-center' }}@endif items-center px-4 py-3 space-x-2.5 w-full h-auto text-sm rounded-md border border-white/10 text-white hover:bg-white/10 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800 transition-all group/country" data-auth="social-provider-button-{{ $slug }}" rel="nofollow">
    <span class="w-5 h-5 transition-all duration-150 ease-linear transform group-hover/country:-rotate-12 group-hover/country:scale-125">
