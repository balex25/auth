        ...
        <div class="flex flex-col justify-center items-start w-full">
            <div class="flex justify-start items-center p-1 w-auto text-xs font-medium rounded-lg bg-zinc-100">
                <div 
                    ...
                    :class="{'bg-white shadow-xs': logo_type == 'image', 'bg-transparent': logo_type != 'image'}"
                    class="shrink-0 px-4 py-2 text-center text-gray-900 rounded-md cursor-pointer">Upload an Image</div>
                    
                <div 
                    x-on:click="logo_type = 'svg'; setTimeout(function(){ document.getElementById('svgTextarea').focus(); }, 10);"
                    :class="{'bg-white shadow-xs': logo_type == 'svg', 'bg-transparent': logo_type != 'svg'}"
                    class="shrink-0 px-4 py-2 text-center text-gray-900 rounded-md cursor-pointer">Use an SVG</div>
            </div>
            <div class="mt-2 w-full bg-white">
                        ...
                        <div class="flex justify-center items-center w-full">
                            <label for="logo_image" class="relative w-full h-auto">
                                ...
                                <div x-show="logo_image" class="flex justify-center items-center pb-2 w-full">
                                    <div class="inline-flex justify-center items-center px-3 py-2 space-x-1.5 text-xs bg-gray-200 rounded-sm cursor-pointer">
                ...
                <div x-show="logo_type == 'svg'" x-data="{ example: false }" class="p-3 rounded-lg border border-gray-200 bg-zinc-50">
                    ...
                    <textarea id="svgTextarea" wire:blur="updateSvg($el.value)" x-model="logo_svg_string" class="mt-3 w-full h-20 font-mono text-xs text-gray-700 bg-white rounded-sm border shadow-xs outline-hidden border-gray-200/60 focus:border-gray-900 focus:ring-2 focus:ring-gray-900 focus:ring-opacity-25 focus:outline-hidden dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-300 dark:focus:border-neutral-500 dark:focus:ring-neutral-500"></textarea>
                </div>
            </div>
        </div>
