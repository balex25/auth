...
new class extends Component
{
    ...
    public function mount()
    {
        ...
        if (! $this->settings->registration_enabled) {
            session()->flash('error', __('auth.register.registrations_disabled', [], app()->getLocale()) !== 'auth.register.registrations_disabled' ? __('auth.register.registrations_disabled') : config('devdojo.auth.language.register.registrations_disabled', 'Registrations are currently disabled.'));
    ...
    public function register()
    {
        ...
        if (! $this->settings->enable_email_registration) {
            session()->flash('error', __('auth.register.email_registration_disabled', [], app()->getLocale()) !== 'auth.register.email_registration_disabled' ? __('auth.register.email_registration_disabled') : config('devdojo.auth.language.register.email_registration_disabled', 'Email registration is currently disabled. Please use social login.'));
...
<x-auth::layouts.app title="{{ $language->register->page_title ?? config('devdojo.auth.language.register.page_title') }}">
    ...
    <x-auth::elements.container>
        ...
        <form wire:submit="register" class="space-y-5">
            ...
            @if($showNameField)
            <x-auth::elements.input :label="$language->register->name ?? config('devdojo.auth.language.register.name')" type="text" wire:model="name" autofocus="true" required />
            @endif
            ...
            @endphp
            <x-auth::elements.input :label="$language->register->email_address ?? config('devdojo.auth.language.register.email_address')" id="email" name="email" type="email" wire:model="email" data-auth="email-input" :autofocus="$autofocusEmail" autocomplete="email" required />
            @endif

            @if($showPasswordField)
           <x-auth::elements.input :label="$language->register->password ?? config('devdojo.auth.language.register.password')" type="password" wire:model="password" id="password" name="password" data-auth="password-input" autocomplete="new-password" required />
            <x-auth::elements.password-requirements />
            @endif

            @if($showPasswordConfirmationField)
            <x-auth::elements.input :label="$language->register->password_confirmation ?? config('devdojo.auth.language.register.password_confirmation')" type="password" wire:model="password_confirmation" id="password_confirmation" name="password_confirmation" data-auth="password-confirmation-input" autocomplete="new-password" required />
            @endif

            <x-auth::elements.button data-auth="submit-button" rounded="md" submit="true">{{ $language->register->button ?? config('devdojo.auth.language.register.button') }}</x-auth::elements.button>
        </form>
        @endif
        ...
        <div class="@if(config('devdojo.auth.settings.social_providers_location') != 'top' && $showEmailRegistration){{ 'mt-3' }}@endif space-x-0.5 text-sm leading-5 @if(config('devdojo.auth.settings.center_align_text')){{ 'text-center' }}@else{{ 'text-left' }}@endif" style="color:{{ config('devdojo.auth.appearance.color.text') }}">
            <span class="opacity-47 dark:text-white">{{ $language->register->already_have_an_account ?? config('devdojo.auth.language.register.already_have_an_account') }}</span>
            <x-auth::elements.text-link data-auth="login-link" href="{{ route('auth.login') }}">{{ $language->register->sign_in ?? config('devdojo.auth.language.register.sign_in') }}</x-auth::elements.text-link>
        </div>
