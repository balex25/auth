...
<x-auth::layouts.app title="{{ __('auth.login.page_title') }}">

    @volt('auth.login')
        <x-auth::elements.container>

            <x-auth::elements.heading
                :text="$language->login->headline"
            ...
            @if($passwordlessLinkSent)
                <x-auth::elements.session-message
                    :message="$language->login->passwordless_sent"
                    type="success"
                />
            @else
                <x-auth::elements.session-message />
            ...
            <form wire:submit="authenticate" class="space-y-5">
                ...
                @else
                    @if($showIdentifierInput)
                        <x-auth::elements.input :label="$language->login->email_address" type="email" wire:model="email" autofocus="true" data-auth="email-input" id="email" name="email" autocomplete="email" required />
                    @endif
                @endif

                @if($showSocialProviderInfo)
                    <div class="p-4 text-sm border rounded-md bg-zinc-50 border-zinc-200">
                        <span>{{ str_replace('__social_providers_list__', implode(', ', $userSocialProviders), $language->login->social_auth_authenticated_message) }}</span>
                        <button wire:click="editIdentity" type="button" class="underline translate-x-0.5">{{ $language->login->change_email }}</button>
                    </div>
                ...
                @php
                    $passwordFieldClasses = $showPasswordField ? 'flex flex-col gap-4' : 'hidden';
                @endphp
